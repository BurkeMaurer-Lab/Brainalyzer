function Brain_CreateGeomFile(outDir, badChannels, prbType, mapType)
    ...
    for i = 1:shankSett.ShankCount
        currShank = shankSett.Shank(i);
        adjacent = [];        
        
        
        totSites = size(currShank.Site, 2);
        ...
        fprintf(pbrFile, ['\t\t\t''channels'': list(range(', num2str(numGood), ')),\n\n']);
        ...
        for j = 1:size(adjacent, 1)
            fprintf(pbrFile, ['\t\t\t\t\t\t(', num2str(adjacent(j, 1)), ', ', num2str(adjacent(j, 2)), '),\n']);
        end
        
        fprintf(pbrFile, '\t\t\t],\n\n');
        fprintf(pbrFile, '# 2D positions of channels, only for visualization purposes\n');
        fprintf(pbrFile, '# in Klustaviewa. The unit does not matter.\n');
        fprintf(pbrFile, '''geometry'': {\n');
        
        for j = 1:size(currShank.Site, 2)
            if ~any(ismember(badChannels, currShank.Site(j).Number))
                fprintf(pbrFile, ['\t\t\t\t\t\t', num2str(currShank.Site(j).ID), ': (']);
                fprintf(pbrFile, [num2str(currShank.Site(j).Geo.x), ', ', num2str(currShank.Site(j).Geo.y)]);
                fprintf(pbrFile, '),\n');
            end
        end
