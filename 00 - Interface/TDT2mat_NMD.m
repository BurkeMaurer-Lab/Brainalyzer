function data = TDT2mat_NMD(tank, block, varargin)
    ...
    if TTX.SelectBlock(['~' block]) ~= 1
        ...
        while strcmp(block_name, '') == 0
            ...
            if strcmp(block_name, block)
                error(['Block found, but problem selecting it: ' block fprintf('\n') ...
...
%                         end
                        %NMD EDIT 2/2/2018
                        data.streams.(varname).data(:,ind:ind+size(temp_data,2)-1) = temp_data; %NMD EDIT 2/2/2018
